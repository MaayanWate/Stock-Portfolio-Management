...
db = client[f"{SERVICE_NAME}"]  # Database name based on service
...
def manage_stocks():
    if request.method == 'GET':
        ...
        # שם מסד הנתונים הנוכחי (התיק הנוכחי)
        current_db = f"{SERVICE_NAME}_db"

        # רשימת מסדי הנתונים לבדיקה, מוציאים את התיק הנוכחי מהבדיקה
        dbs_to_check = [db_name for db_name in ["stocks1_db", "stocks2_db"] if db_name != current_db]
        print(f"Current database: {current_db}, checking against: {dbs_to_check}")
        ...
        stocks_collection.insert_one(stock)  # שמירת המניה במסד הנתונים
